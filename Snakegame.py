...
class SnakeGame:
    ...
    def __init__(self):
        ...
        # to initialize the snake in the range of (x1,y1,x2,y1)初始化蛇长                
        r=random.randrange(100,800,self.step)#指定递增基数集合中的一个随机数
        self.snakeX=[r,r+self.step,r+self.step*2]
        self.snakeY=[r,r,r]#为什么是三维坐标
        
        # to initialize the moving direction
        self.snakeDirection = 'left'  
        self.snakeMove = [-1,0]#为什么是一个列表
        ...
        self.score_label=Label(self.frame2,text="Score: 0")#单行文本
        
        self.frame1.pack()
        self.frame2.pack(fill=BOTH)
        self.score_label.pack(side=LEFT)
        self.canvas.pack(fill=BOTH)
        #五个墙、分数、食物、蛇身、和游戏运行的函数
        self.draw_wall()
        self.draw_score()
        self.draw_food()
        self.draw_snake()
    ...
    "=== View Part ==="        
    def draw_wall(self):#显示墙壁
        self.canvas.create_line(10,10,582,10,fill='blue',width=5)
        self.canvas.create_line(582,10,582,359,fill='blue',width=5)
        self.canvas.create_line(582,359,10,359,fill='blue',width=5)
        self.canvas.create_line(10,359,10,10,fill='blue',width=5)   
        
    
    def draw_score(self):#显示分数
        self.score()                        # score model
        self.score_label.config(self.score_label,text="Score: "+str(self.gamescore))
        
    def draw_food(self):#显示食物
        self.canvas.delete("food")
        self.foodx,self.foody=self.random_food()    #food model
        self.canvas.create_rectangle(self.foodx,self.foody,self.foodx+self.step\
        ,self.foody+self.step,fill='red' ,tags="food")     #food view

    def draw_snake(self):#显示蛇身
        ...
        for i in range(len(x)):             # snake view
            ...
            , fill='brown',tags='snake')    
    
    "=== Model Part ==="
    # food model
    def random_food(self):   #随机生成食物位置   
    ...
    #score model    
    def score(self):#生成分数,每吃一个加10分
        
        self.gamescore += 10
        
    
    "=== Control Part ==="     
    def iseated(self):#判断食物是否被吃
    ...
    def isdead(self):#若死亡返回True，否则返回Flase
